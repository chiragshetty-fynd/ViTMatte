...
class MattingCriterion(nn.Module):
    ...
    def loss_gradient_penalty(self, sample_map, preds, targets):
        ...
        # gradient in x
        sobel_x_kernel = torch.tensor([[[[-1, 0, 1], [-2, 0, 2], [-1, 0, 1]]]]).type(
            dtype=preds.type()
        )
        ...
        # gradient in y
        sobel_y_kernel = torch.tensor([[[[-1, -2, -1], [0, 0, 0], [1, 2, 1]]]]).type(
            dtype=preds.type()
        )
    ...
    def unknown_l1_loss(self, sample_map, preds, targets):
        ...
        loss = (
            F.l1_loss(preds["phas"] * sample_map, targets["phas"] * sample_map) * scale
        )
    ...
    def known_l1_loss(self, sample_map, preds, targets):
        ...
        loss = (
            F.l1_loss(preds["phas"] * new_sample_map, targets["phas"] * new_sample_map)
            * scale
        )
