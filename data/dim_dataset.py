...
class Prefetcher:
    ...
    def __next__(self):
        ...
        if sample is not None:
            ...
            raise StopIteration(
                "No samples in loader. example: `iterator = iter(Prefetcher(loader)); "
                "data = next(iterator)`"
            )
        return sample
...
class BaseImageFile(object):
    ...
    @staticmethod
    def _list_abspath(data_dir):
        return sorted(glob.glob(f'{data_dir}/**/*.jpg', recursive=True) + glob.glob(f'{data_dir}/**/*.jpeg', recursive=True))
...
#         self.valid_bg_list = [
#             os.path.splitext(name)[0] for name in os.listdir(self.bg_dir)
#         ]
...
class ImageFile(BaseImageFile):
    def __init__(
        ...
        self.images = self._list_abspath(self.images_dir)
        self.alphas = self._list_abspath(self.alphas_dir)
        self.trimaps = self._list_abspath(self.trimaps_dir)
...
class ToTensor(object):
    ...
    def __call__(self, sample):
        ...
        sample["image"], sample["alpha"], sample["trimap"] = (
            ...
            torch.from_numpy(trimap).to(torch.long),
        )
        sample["image"] = sample["image"]

        if CONFIG.model.trimap_channel == 3:
            sample["trimap"] = (
                F.one_hot(sample["trimap"], num_classes=3).permute(2, 0, 1).float()
            )
...
class RandomCrop(object):
    ...
    def __call__(self, sample):
        ...
        if w < self.output_size[0] + 1 or h < self.output_size[1] + 1:
            ratio = (
                1.1 * self.output_size[0] / h
                if h < w
                else 1.1 * self.output_size[1] / w
            )
        ...
        unknown_list = list(
            zip(
                *np.where(
                    small_trimap[
                        self.margin // 4 : (h - self.margin) // 4,
                        self.margin // 4 : (w - self.margin) // 4,
                    ]
                    == 128
                )
            )
        ...
        if len(np.where(trimap == 128)[0]) == 0:
            self.logger.error(
                "{} does not have enough unknown area for crop. Resized to target size."
...
class GenMask(object):
    ...
    def __call__(self, sample):
        ...
        max_kernel_size = 30
        alpha = cv2.resize(
            alpha_ori, (640, 640), interpolation=maybe_random_interp(cv2.INTER_NEAREST)
        )
...
class CutMask(object):
    ...
    def __call__(self, sample):
        ...
        h, w = mask.shape
        perturb_size_h, perturb_size_w = random.randint(h // 4, h // 2), random.randint(
            w // 4, w // 2
        )
        ...
        mask[x : x + perturb_size_h, y : y + perturb_size_w] = mask[
            x1 : x1 + perturb_size_h, y1 : y1 + perturb_size_w
        ].copy()
...
class DataGenerator(Dataset):
    def __init__(self, data):
        ...
        # train_trans = val_trans = test_trans = [OriginScale(), ToTensor()]
        ...
        self.transform = transforms.Compose([OriginScale(), ToTensor()])
    ...
    def __getitem__(self, idx):
        ...
        alpha = cv2.imread(self.alphas[idx], 0) / 255.0
        trimap = cv2.imread(self.trimaps[idx], 0)
        mask = (trimap >= 170).astype(np.float32)
        image_name = os.path.split(self.images[idx])[-1]
    ...
    def _composite_fg(self, fg, alpha, idx):
        if np.random.rand() < 0.5:
            ...
            alpha2 = (
                cv2.imread(self.alpha[idx2 % self.fg_num], 0).astype(np.float32) / 255.0
            )
            h, w = alpha.shape
            fg2 = cv2.resize(
